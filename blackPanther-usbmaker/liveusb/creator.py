...
class LiveUSBCreator(object):
    ...
    label = "PANTHER-USB"    # if one doesn't already exist
...
#        self.log.info('OLPC/args: %s' % args)
        if not os.path.exists(os.path.join(self.dest, 'boot')):
...
#        self.log.info('OLPC END: %s' % olpc_cfg.name)

    def get_kernel_args(self):
        """ Grab the kernel arguments from our syslinux configuration """
        args = []
        cfg = file(os.path.join(self.dest, 'boot', 'syslinux', 'syslinux.cfg'))
    ...
    def verify_iso_sha1(self, progress=None):
        ...
        if release:
            ...
            if 'sha1' in release:
                self.log.info(_("Verifying SHA1 checksum of blackPanther CD image..."))
                hash = 'sha1'
                checksum = hashlib.sha1()
            elif 'sha256' in release:
                self.log.info(_("Verifying SHA256 checksum of blackPanher CD image..."))
                hash = 'sha256'
            ...
            if checksum.hexdigest() == release[hash]:
                ...
                self.log.info(_("Error: The SHA1 of your blackPanther CD is "
...
#                           % (self.get_overlay(), self.overlay))
    ...
    def delete_liveos(self):
        ...
        self.log.info(_('Removing existing OS from device'))
        for path in [self.get_liveos(),
                     os.path.join(self.dest + os.path.sep, 'boot'),
                     os.path.join(self.dest + os.path.sep, 'syslinux'),
                     os.path.join(self.dest + os.path.sep, 'loopbacks'),
            ...
            if os.path.exists(path):
                ...
                try:
                    ...
                    raise LiveUSBError(_("Unable to remove previous OS: "
                                         "%s" % str(e)))
    ...
    def get_liveos(self):
        return os.path.join(self.dest + os.path.sep, "PANTHER-USB")
...
#                        self.popen('/sbin/dosfslabel %s %s' % (self.drive['device'], self.label))
    ...
    def extract_iso(self):
        ...
        #self.log.info(_(tmpdir))
        self.popen('mount -o loop,ro "%s" %s' % (self.iso, tmpdir))
        tmpliveos = os.path.join(tmpdir)
        # ! tmpliveos = os.path.join(tmpdir, 'LiveOS')
        try:
            if not os.path.isdir(tmpliveos):
                raise LiveUSBError(_("Unable to find blackPanther OS on ISO"))
            liveos = os.path.join(self.dest)
            tmploop = os.path.join(self.dest, 'loopbacks')
            if not os.path.exists(tmploop):
                os.mkdir(tmploop)

#            if os.path.exists(os.path.join(tmpdir, 'boot')):
#                boot = os.path.join(self.dest, 'boot')
#                if not os.path.exists(boot):
#                    os.mkdir(boot)

#            self.popen("sleep 100")
            if os.path.isfile(os.path.join(tmpdir, 'livecd.sqfs')):
        	#img = os.path.join(tmpdir, 'livecd.sqfs')
        	#for img in ('livecd.sqfs'):
#                start = datetime.now()
#                self.log.info(_("Skip Extract..."))
                
              self.popen("cp -f %s '%s'" % (os.path.join(tmpdir, 'livecd.sqfs'),
                                           os.path.join(tmploop, 'livecd.sqfs')))
...
#                                      self.mb_per_sec)
    	    ...
    	    #if not os.path.exists(boot):
            #	    os.mkdir(boot)
        	#self.popen("cp %s/* '%s'" % (os.path.join(tmpdir, 'boot'),
            #                           isolinux))
...
#        tmpdir = tempfile.mkdtemp()
    ...
    def install_bootloader(self):
        ...
        syslinux_path = os.path.join(self.dest, "boot/syslinux")
...
#        os.unlink(os.path.join(syslinux_path, "isolinux.cfg"))
        ...
        for ldlinux in [os.path.join(self.drive['device'] + os.path.sep, p, 'ldlinux.sys')
                        for p in (syslinux_path, '')]:
        ...
        if self.drive['fstype'] in ('ext2', 'ext3'):
            ...
            self.popen('syslinux%s%s -d %s %s' %  (self.opts.force and
                      ...
                      'boot/syslinux', self.drive['device']))
#           self.popen('syslinux%s%s -m -a -d %s %s' %  (self.opts.force and ' -f' 
#                                                         or '', self.opts.safe and ' -s' or '', 'boot//syslinux', self.drive['device']))
...
class WindowsLiveUSBCreator(LiveUSBCreator):
    ...
    def gen_datafile(self):
        """ Run generate storage image """
        self.log.info(_("Generate blackPanther storagefile on device..."))
        self.popen('dd if=/dev/zero of="%s" count=1024k bs=1M' % (self.dest+'\\loopbacks\\data'))
        self.log.info(_("Formating the blackPanther storagefile for usage..."))
        self.popen('mke2fs -L LOOP -F "%s"' % (self.dest+'\\loopbacks\\data'))
    ...
    def calculate_liveos_checksum(self):
        ...
        for img in (os.path.join('loopbacks','livecd.sqfs'),
                    os.path.join('boot', 'initrd.gz'),
                    os.path.join('boot', 'initrd2.gz'),
                    os.path.join('boot', 'vmlinuz'),
                    os.path.join('boot', 'vmlinuz2'),
                    os.path.join('boot', 'syslinux',  'bootlogo')):
    ...
    def format_device(self):
        ...
        self.popen('format /Q /X /y /V:PANTHER-USB /FS:FAT32 %s' % self.drive['device'])
