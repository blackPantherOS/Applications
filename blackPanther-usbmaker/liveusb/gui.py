...
class ReleaseDownloader(QtCore.QThread):
    ...
    def run(self):
        self.emit(QtCore.SIGNAL("status(PyQt_PyObject)"), (_("Downloading: ")+os.path.basename(self.url)+("...")))
...
class USB_Downloader(QtCore.QThread):

    def __init__(self, progress, proxies):
        QtCore.QThread.__init__(self)
        self.url = "http://eu.blackpanther.hu/?file_id=8"
        self.progress = progress
        self.proxies = proxies
...
class LiveUSBThread(QtCore.QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.live.copy_bootdir()
            #self.live.gen_datafile()
...
#            self.progress.terminate()
            self.status(_("Complete the mission! Duration:")+duration)

        except Exception, e:
            self.status(e.args[0])
            self.status(_("blackPanther USB creation failed!"))
